"""One-off driver: resolve + download Maybank's latest annual report and SAVE the PDF."""
import sys
sys.stdout.reconfigure(encoding="utf-8")
import ir_resolve_proto as R
import ir_fetch_proto as F

NAME, TICKER, COUNTRY = "Malayan Banking Berhad", "MAYBANK", "Malaysia"
SAVE_PATH = "_ir_MAYBANK.pdf"
...
if not result:
    print("  ABSTAIN — no gate-passing annual report found (will try CDN-via-search fallback)")
else:
    info = result["info"]
    print(f"\n  *** SAVED -> {SAVE_PATH}  (FY{result['fiscal_year']}, {info['pages']} pages) ***")
    print(f"      url={result['url']}\n      sbc_hits={info['sbc_hits']}")
